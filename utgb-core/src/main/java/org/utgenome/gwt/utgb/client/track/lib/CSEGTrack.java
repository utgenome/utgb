 ...
 * </pre>
 * 
 * What follows explains the properties.
 * Image height property is the height of an image generated by CSEGTrack. If you do not
 * like a scroll bar, you should keep it smaller than the height of the track,
 * but still you may set a larger value to it.
 * Color array property is comma-separated values, each of which is a hexadecimal value
 * of 6 letters in a form like RRGGBB (Let's imagine you are writing HTML. It's basically
 * the same although without '#'). You should not include any space character in the color
 * array property. We had not trimmed spaces.
 * Scale color is the color of the scale. When draw scale property is false, the scale
 * will not be drew. The scale is a set of lines parallel to each other. The distance of
 * the two neighboring scale lines is 'Samples per scale' times 'Height per sample'.
 * Height per sample property is the number of pixels per sample.
 * Priority indicies property is comma-separated values (again, without any spaces).  
 * 
 * 
 * @author mkasa
 * 
...
public class CSEGTrack extends GenomeTrack {
	...
	public void onChangeTrackConfig(TrackConfigChange change) {
		if (change.containsOneOf(new String[]{
				CONFIG_PATH, CONFIG_HEIGHT_PER_SAMPLE, CONFIG_TOTAL_HEIGHT, CONFIG_COLOR_ARRAY,
				CONFIG_SCALE_COLOR, CONFIG_SAMPLES_PER_SCALE_LINE, CONFIG_PRIORITY_INDEX, CONFIG_DRAW_SCALE})) {
			refresh();
		}
	}
