...
/**
 * A modified version of the animation support class included in GWT. An {@link Animation} is a continuous event that
 * updates progressively over time at a non-fixed frame rate.
 */
public abstract class Animation {
	...
	protected Animation(int default_frame_delay) {
		DEFAULT_FRAME_DELAY = default_frame_delay;
	}

	/**
	 * The default time in milliseconds between frames.
	 */
	private final int DEFAULT_FRAME_DELAY;
	...
	/**
	 * Is the {@link Animation} running, even if {@link #onStart()} has not yet been called.
	 */
	...
	/**
	 * Immediately cancel this animation. If the animation is running or is scheduled to run, {@link #onCancel()} will
	 * be called.
	 */
	public void cancel() {
	...
	/**
	 * Run this animation at the given startTime. If the startTime has already passed, the animation will be synchronize
	 * as if it started at the specified start time. If the animation is already running, it will be canceled first.
	 * 
	 * @param duration
	 *            the duration of the animation in milliseconds
	 * @param startTime
	...
	/**
	 * Called immediately after the animation is canceled. The default implementation of this method calls
	 * {@link #onComplete()} only if the animation has actually started running.
	 */
	 ...
	 * 
	 * The value of progress is between 0.0 and 1.0 inclusively (unless you override the {@link #interpolate(double)}
	 * method to provide a wider range of values). You can override {@link #onStart()} and {@link #onComplete()} to
	 * perform setup and tear down procedures.
	 */
